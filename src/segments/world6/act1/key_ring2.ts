...
export default class key_ring2 extends Phaser.GameObjects.Container {
	...
	constructor(scene: Phaser.Scene, x?: number, y?: number) {
		...
		// collect
		const collect = new Collect(scene, 716, -30, "ring2");
		this.add(collect);
		...
		// nineslice
		const nineslice = scene.add.nineslice(492, -78, "stone1", undefined, 500, 16, 4, 4, 4, 4) as Phaser.GameObjects.NineSlice & { body: Phaser.Physics.Arcade.Body };
		...
		// lists
		const platforms = [rectangle_1, nineslice, nineslice_1];
